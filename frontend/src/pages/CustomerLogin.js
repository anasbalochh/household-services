...
const CustomerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      try {
        const response = await axios.post('http://localhost:5000/api/auth/login', { email, password, role: 'customer' });
        dispatch(login({ token: response.data.token, role: response.data.role, email }));
        navigate('/customer-dashboard'); // Default redirect, booking details after booking
      } catch (err) {
        setError(err.response?.data?.message || 'Login failed');
      }
    } else {
      try {
        const response = await axios.post('http://localhost:5000/api/auth/register', { name, email, password, role: 'customer' });
        dispatch(login({ token: response.data.token, role: response.data.role, email }));
        navigate('/customer-dashboard'); // Default redirect
      } catch (err) {
